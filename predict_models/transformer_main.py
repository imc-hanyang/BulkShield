"""
Transformer-Based Fraud Detection Training Script

This module implements the complete training pipeline for a Transformer-based
fraud detection classifier optimized for H100 GPUs. It handles data loading,
vocabulary construction, training with mixed precision, and model evaluation.

Pipeline Overview:
    1. Load user transaction sequence file paths and labels
    2. Build vocabulary for categorical features from training subset
    3. Create TransformerDataset for lazy-loading sequences
    4. Train SRTTransformerClassifier with AdamW optimizer
    5. Evaluate using Macro F1 score, save best checkpoint

Key Features:
    - Lazy data loading: Memory-efficient CSV parsing per sample
    - H100 optimization: Mixed precision, high worker count, persistent workers
    - Time delta encoding: Discretized inter-event time buckets
...
Output:
    - Model checkpoints saved to eval_results/dl/<timestamp>/
    - Vocabulary JSON file for inference consistency
    - Detailed evaluation metrics and logs
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("transformer_main.log"),
...
def fast_parse_ts(ts_str: str):
    ...
    Args:
        ts_str: Timestamp string in various common formats.
...
class TransformerDataset(Dataset):
    """
    PyTorch Dataset for lazy-loading transformer input sequences.
    
    Loads individual user CSV files on demand, parses features, computes
    time deltas, and applies vocabulary encoding for categorical columns.
    
    Args:
        input_paths: List of paths to user sequence CSV files.
        labels: Corresponding binary labels (0=normal, 1=fraud).
        vocabs: Vocabulary dictionaries for categorical encoding.
        cfg: CFG dataclass with model configuration.
    
    Returns per sample:
        - cat: Categorical features [T, num_cat_cols] as vocab indices
        - num: Numeric features [T, num_num_cols] with log1p scaling
        - delta: Time delta buckets [T] for inter-event encoding
        - y: Label tensor
    """
...
def collate_fn(batch):
    """
    Collate function for batching variable-length Transformer sequences.
    
    Pads all sequences in the batch to the maximum length and creates
    a padding mask for attention computation.
    
    Args:
        batch: List of sample dictionaries from TransformerDataset.
        
    Returns:
        Batched tensors: cat [B,T,C], num [B,T,N], delta [B,T], pad_mask [B,T], y [B]
    """
    ...
    # Initialize padded tensors (zeros for features, True for mask)
    ...
    pad_mask = torch.ones(len(batch), maxT, dtype=torch.bool)  # True = padding position
...
def build_vocabs_on_the_fly(paths, limit=5000):
    """
    Build vocabulary from a subset of data to avoid full scan.
    For production, should load precomputed vocabs.
    """
    ...
    if IS_INTERACTIVE:
        iter_wrapper = tqdm(sample_paths, desc="Vocab Building", file=sys.stdout)
    else:
        iter_wrapper = sample_paths
        logger.info(f"Vocab Building: Processing {len(sample_paths)} files...")
    
    for i, p in enumerate(iter_wrapper):
        if not IS_INTERACTIVE and i % 1000 == 0 and i > 0:
...
def train_one_epoch(model, loader, optimizer, criterion, device, epoch):
    ...
    Args:
        model: SRTTransformerClassifier model.
...
def evaluate(model, loader, device):
    ...
    Returns:
        Tuple of (true_labels, predicted_probabilities) for class 1 (fraud).
    """
    model.eval()
    all_y = []
    all_probs = []  # Probabilities of fraud class
...
def main():
    """
    Main training pipeline for Transformer fraud classifier.
    
    Orchestrates the complete training workflow:
        1. Load train/test data paths
        2. Build vocabulary from training subset
        3. Create datasets and dataloaders
        4. Initialize model with H100-optimized config
        5. Train with early stopping based on Macro F1
        6. Save best model checkpoint and vocabulary
    """
    # Hardware setup
    ...
    # Check if vocab already exists? For now, build on fly for robustness or load if I implemented saving.
    # To keep it simple and effective: build from subset.
    vocabs = build_vocabs_on_the_fly(train_paths, limit=20000) 

    # 3. Dataset & DataLoader (High num_workers for 100 Cores)
    # 100 cores -> num_workers=32 is safe, maybe more depending on I/O
    num_workers = 32
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        metrics = print_evaluation_report(
            "transformer", y_true, y_scores, threshold=0.5, 
        ...
        if macro_f1 > best_f1:
            ...
            # Immediate Save
            save_dir = get_run_dir('dl', timestamp)
            torch.save(model.state_dict(), os.path.join(save_dir, f"transformer_best_{best_f1:.4f}.pth"))
    ...
    # 6. Final Save
    # 6. Final Evaluation (Optional: just to see final stats of best model)
    if best_model_state:
        model.load_state_dict(best_model_state)
        y_true, y_scores = evaluate(model, val_loader, device)
        print_evaluation_report(
            "transformer", y_true, y_scores, threshold=0.5, 
            category='dl', save=True, timestamp=timestamp
        )
