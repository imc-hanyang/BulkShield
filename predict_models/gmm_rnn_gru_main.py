...
from data_utils import LazyDataset, pad_collate_fn
...
CONFIG = {
    ...
    # Paths (GMM)
    'train_label_dir': '/home/srt/Dataset/label/label_train_per_user',
    'test_label_dir': '/home/srt/Dataset/label/label_test_per_user',
    # Sequence Data is shared
...
def load_gmm_data(label_dir, seq_dir, split_name="Train"):
    print(f"\n[{split_name}] Mapping Data Files (GMM)...")
    ...
    for user_id in tqdm(common_users, ncols=100, mininterval=1.0):
        ...
        try:
            with open(lbl_path, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    # GMM Labels typically use 'label' or 'is_fraud'
                    val = 0
                    if 'label' in row:
                        val = int(float(row['label']))
                    elif 'is_fraud' in row:
                         val = int(float(row['is_fraud']))
                    elif 'is_risk' in row: # Just in case
...
def run_model_training(model_name, train_loader, val_loader, timestamp):
    ...
    print(f"\n{'='*50}")
    print(f"🚀 Training Model (GMM): {model_name.upper()}")
    print(f"{'='*50}")
    ...
    SAVE_DIR = f'/home/srt/ml_results/{base_name}/{timestamp}'
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        print_evaluation_report(
            ...
            category='gmm_dl', save=False, timestamp=timestamp
        )
...
def main():
    ...
    # 1. Load Data
    print("📥 Loading Datasets (GMM)...")
    train_paths, train_labels = load_gmm_data(CONFIG['train_label_dir'], CONFIG['train_seq_dir'], "Train")
    test_paths, test_labels = load_gmm_data(CONFIG['test_label_dir'], CONFIG['test_seq_dir'], "Test")
